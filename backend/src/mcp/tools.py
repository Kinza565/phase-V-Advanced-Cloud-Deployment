"""
MCP tools for task management.
Wraps existing task service logic for AI agent consumption.
Supports Phase 5 features: priorities, tags, due dates, reminders, recurrence.
"""
from typing import List, Optional, Dict, Any
from uuid import UUID
from sqlmodel import Session
import logging
from services.tasks import TasksService
from models.task import Task, Priority, Recurrence

logger = logging.getLogger(__name__)

def add_task(
    session: Session,
    user_id: UUID,
    title: str,
    description: Optional[str] = None,
    priority: Optional[str] = None,
    due_date: Optional[str] = None,
    remind_at: Optional[str] = None,
    recurrence: Optional[str] = None,
    tags: Optional[List[str]] = None
) -> Dict[str, Any]:
    """
    Create a new task with Phase 5 features.

    Args:
        session: Database session
        user_id: User ID
        title: Task title
        description: Optional description
        priority: Priority level (low, medium, high)
        due_date: Due date (natural language or ISO format)
        remind_at: Reminder time (natural language or ISO format)
        recurrence: Recurrence pattern (none, daily, weekly, monthly)
        tags: List of tag names
    """
    logger.info(f"Tool: add_task called for user {user_id} with title='{title}'")
    try:
        task = TasksService.create_task(
            session=session,
            user_id=user_id,
            title=title,
            description=description,
            priority=priority,
            due_date=due_date,
            remind_at=remind_at,
            recurrence=recurrence,
            tags=tags
        )
        return {
            "success": True,
            "message": f"Task '{task.title}' created successfully.",
            "task": {
                "id": str(task.id),
                "title": task.title,
                "description": task.description,
                "is_completed": task.is_completed,
                "priority": task.priority.value,
                "due_date": task.due_date.isoformat() if task.due_date else None,
                "remind_at": task.remind_at.isoformat() if task.remind_at else None,
                "recurrence": task.recurrence.value,
                "tags": [tag.name for tag in task.tags] if task.tags else []
            }
        }
    except Exception as e:
        return {"success": False, "message": f"Failed to create task: {str(e)}"}

def list_tasks(
    session: Session,
    user_id: UUID,
    completed: Optional[bool] = None
) -> Dict[str, Any]:
    """
    List all tasks for the user.
    """
    logger.info(f"Tool: list_tasks called for user {user_id} with completed={completed}")
    try:
        tasks = TasksService.get_user_tasks(session, user_id)
        
        # Filter if requested
        if completed is not None:
            tasks = [t for t in tasks if t.is_completed == completed]
            
        task_list = [
            {
                "id": str(t.id),
                "title": t.title,
                "description": t.description,
                "is_completed": t.is_completed
            }
            for t in tasks
        ]
        
        return {
            "success": True,
            "count": len(task_list),
            "tasks": task_list
        }
    except Exception as e:
        return {"success": False, "message": f"Failed to list tasks: {str(e)}"}

def _find_task(session: Session, user_id: UUID, task_identifier: str) -> Optional[Task]:
    """Helper to find task by ID or fuzzy title match."""
    # Try as UUID first
    try:
        task_id = UUID(task_identifier)
        return TasksService.get_task_by_id(session, task_id, user_id)
    except:
        pass
        
    # Fuzzy match by title
    tasks = TasksService.get_user_tasks(session, user_id)
    matches = [t for t in tasks if task_identifier.lower() in t.title.lower()]
    
    if len(matches) == 1:
        return matches[0]
    return None

def complete_task(
    session: Session,
    user_id: UUID,
    task_identifier: str
) -> Dict[str, Any]:
    """
    Mark a task as completed.
    Accepts UUID or partial title.
    """
    logger.info(f"Tool: complete_task called for user {user_id} with task_identifier='{task_identifier}'")
    try:
        task = _find_task(session, user_id, task_identifier)
        if not task:
            return {"success": False, "message": f"Task '{task_identifier}' not found or ambiguous."}
            
        task = TasksService.update_task(session, task.id, user_id, is_completed=True)
        return {
            "success": True,
            "message": f"Task '{task.title}' marked as completed.",
            "task": {"id": str(task.id), "is_completed": True}
        }
    except Exception as e:
        return {"success": False, "message": f"Failed to complete task: {str(e)}"}

def update_task(
    session: Session,
    user_id: UUID,
    task_identifier: str,
    title: Optional[str] = None,
    description: Optional[str] = None,
    priority: Optional[str] = None,
    due_date: Optional[str] = None,
    remind_at: Optional[str] = None,
    recurrence: Optional[str] = None
) -> Dict[str, Any]:
    """
    Update a task with Phase 5 features.
    """
    logger.info(f"Tool: update_task called for user {user_id} with task_identifier='{task_identifier}'")
    try:
        task = _find_task(session, user_id, task_identifier)
        if not task:
            return {"success": False, "message": f"Task '{task_identifier}' not found or ambiguous."}

        task = TasksService.update_task(
            session,
            task.id,
            user_id,
            title=title,
            description=description,
            priority=priority,
            due_date=due_date,
            remind_at=remind_at,
            recurrence=recurrence
        )
        return {
            "success": True,
            "message": f"Task '{task.title}' updated.",
            "task": {
                "id": str(task.id),
                "title": task.title,
                "description": task.description,
                "priority": task.priority.value,
                "due_date": task.due_date.isoformat() if task.due_date else None,
                "remind_at": task.remind_at.isoformat() if task.remind_at else None,
                "recurrence": task.recurrence.value,
                "tags": [tag.name for tag in task.tags] if task.tags else []
            }
        }
    except Exception as e:
        return {"success": False, "message": f"Failed to update task: {str(e)}"}

def delete_task(
    session: Session,
    user_id: UUID,
    task_identifier: str
) -> Dict[str, Any]:
    """
    Delete a task.
    """
    logger.info(f"Tool: delete_task called for user {user_id} with task_identifier='{task_identifier}'")
    try:
        task = _find_task(session, user_id, task_identifier)
        if not task:
            return {"success": False, "message": f"Task '{task_identifier}' not found or ambiguous."}

        TasksService.delete_task(session, task.id, user_id)
        return {
            "success": True,
            "message": f"Task '{task.title}' deleted."
        }
    except Exception as e:
        return {"success": False, "message": f"Failed to delete task: {str(e)}"}

# Phase 5: Priority Management
def set_task_priority(
    session: Session,
    user_id: UUID,
    task_identifier: str,
    priority: str
) -> Dict[str, Any]:
    """
    Set the priority of a task.
    """
    logger.info(f"Tool: set_task_priority called for user {user_id} with task='{task_identifier}', priority='{priority}'")
    try:
        task = _find_task(session, user_id, task_identifier)
        if not task:
            return {"success": False, "message": f"Task '{task_identifier}' not found or ambiguous."}

        task = TasksService.update_task(session, task.id, user_id, priority=priority)
        return {
            "success": True,
            "message": f"Task '{task.title}' priority set to {task.priority.value}.",
            "task": {
                "id": str(task.id),
                "priority": task.priority.value
            }
        }
    except Exception as e:
        return {"success": False, "message": f"Failed to set priority: {str(e)}"}

# Phase 5: Tag Management
def add_task_tag(
    session: Session,
    user_id: UUID,
    task_identifier: str,
    tag_name: str
) -> Dict[str, Any]:
    """
    Add a tag to a task.
    """
    logger.info(f"Tool: add_task_tag called for user {user_id} with task='{task_identifier}', tag='{tag_name}'")
    try:
        task = _find_task(session, user_id, task_identifier)
        if not task:
            return {"success": False, "message": f"Task '{task_identifier}' not found or ambiguous."}

        task = TasksService.add_tag_to_task(session, task.id, user_id, tag_name)
        return {
            "success": True,
            "message": f"Tag '{tag_name}' added to task '{task.title}'.",
            "task": {
                "id": str(task.id),
                "tags": [tag.name for tag in task.tags] if task.tags else []
            }
        }
    except Exception as e:
        return {"success": False, "message": f"Failed to add tag: {str(e)}"}

def remove_task_tag(
    session: Session,
    user_id: UUID,
    task_identifier: str,
    tag_name: str
) -> Dict[str, Any]:
    """
    Remove a tag from a task.
    """
    logger.info(f"Tool: remove_task_tag called for user {user_id} with task='{task_identifier}', tag='{tag_name}'")
    try:
        task = _find_task(session, user_id, task_identifier)
        if not task:
            return {"success": False, "message": f"Task '{task_identifier}' not found or ambiguous."}

        task = TasksService.remove_tag_from_task(session, task.id, user_id, tag_name)
        return {
            "success": True,
            "message": f"Tag '{tag_name}' removed from task '{task.title}'.",
            "task": {
                "id": str(task.id),
                "tags": [tag.name for tag in task.tags] if task.tags else []
            }
        }
    except Exception as e:
        return {"success": False, "message": f"Failed to remove tag: {str(e)}"}

# Phase 5: Due Date Management
def set_task_due_date(
    session: Session,
    user_id: UUID,
    task_identifier: str,
    due_date: str
) -> Dict[str, Any]:
    """
    Set the due date of a task.
    """
    logger.info(f"Tool: set_task_due_date called for user {user_id} with task='{task_identifier}', due_date='{due_date}'")
    try:
        task = _find_task(session, user_id, task_identifier)
        if not task:
            return {"success": False, "message": f"Task '{task_identifier}' not found or ambiguous."}

        task = TasksService.update_task(session, task.id, user_id, due_date=due_date)
        return {
            "success": True,
            "message": f"Due date set for task '{task.title}'.",
            "task": {
                "id": str(task.id),
                "due_date": task.due_date.isoformat() if task.due_date else None
            }
        }
    except Exception as e:
        return {"success": False, "message": f"Failed to set due date: {str(e)}"}

# Phase 5: Reminder Management
def set_task_reminder(
    session: Session,
    user_id: UUID,
    task_identifier: str,
    remind_at: str
) -> Dict[str, Any]:
    """
    Set a reminder for a task.
    """
    logger.info(f"Tool: set_task_reminder called for user {user_id} with task='{task_identifier}', remind_at='{remind_at}'")
    try:
        task = _find_task(session, user_id, task_identifier)
        if not task:
            return {"success": False, "message": f"Task '{task_identifier}' not found or ambiguous."}

        task = TasksService.set_reminder(session, task.id, user_id, remind_at)
        return {
            "success": True,
            "message": f"Reminder set for task '{task.title}'.",
            "task": {
                "id": str(task.id),
                "remind_at": task.remind_at.isoformat() if task.remind_at else None
            }
        }
    except Exception as e:
        return {"success": False, "message": f"Failed to set reminder: {str(e)}"}

# Phase 5: Recurrence Management
def set_task_recurrence(
    session: Session,
    user_id: UUID,
    task_identifier: str,
    recurrence: str
) -> Dict[str, Any]:
    """
    Set the recurrence pattern for a task.
    """
    logger.info(f"Tool: set_task_recurrence called for user {user_id} with task='{task_identifier}', recurrence='{recurrence}'")
    try:
        task = _find_task(session, user_id, task_identifier)
        if not task:
            return {"success": False, "message": f"Task '{task_identifier}' not found or ambiguous."}

        task = TasksService.update_task(session, task.id, user_id, recurrence=recurrence)
        return {
            "success": True,
            "message": f"Recurrence set to {task.recurrence.value} for task '{task.title}'.",
            "task": {
                "id": str(task.id),
                "recurrence": task.recurrence.value
            }
        }
    except Exception as e:
        return {"success": False, "message": f"Failed to set recurrence: {str(e)}"}

# Phase 5: Advanced Filtering
def list_tasks_with_filters(
    session: Session,
    user_id: UUID,
    priority: Optional[str] = None,
    tag: Optional[str] = None,
    completed: Optional[bool] = None,
    overdue: Optional[bool] = None,
    sort_by: Optional[str] = None,
    sort_order: Optional[str] = None
) -> Dict[str, Any]:
    """
    List tasks with advanced Phase 5 filters.
    """
    logger.info(f"Tool: list_tasks_with_filters called for user {user_id}")
    try:
        tasks = TasksService.get_user_tasks(
            session=session,
            user_id=user_id,
            priority=priority,
            tag=tag,
            is_completed=completed,
            overdue=overdue,
            sort_by=sort_by,
            sort_order=sort_order
        )

        task_list = [
            {
                "id": str(t.id),
                "title": t.title,
                "description": t.description,
                "is_completed": t.is_completed,
                "priority": t.priority.value,
                "due_date": t.due_date.isoformat() if t.due_date else None,
                "remind_at": t.remind_at.isoformat() if t.remind_at else None,
                "recurrence": t.recurrence.value,
                "tags": [tag.name for tag in t.tags] if t.tags else []
            }
            for t in tasks
        ]

        return {
            "success": True,
            "count": len(task_list),
            "tasks": task_list
        }
    except Exception as e:
        return {"success": False, "message": f"Failed to list tasks: {str(e)}"}

# Phase 5: Search
def search_tasks(
    session: Session,
    user_id: UUID,
    query: str
) -> Dict[str, Any]:
    """
    Search tasks by keyword.
    """
    logger.info(f"Tool: search_tasks called for user {user_id} with query='{query}'")
    try:
        tasks = TasksService.search_tasks(session, user_id, query)

        task_list = [
            {
                "id": str(t.id),
                "title": t.title,
                "description": t.description,
                "is_completed": t.is_completed,
                "priority": t.priority.value,
                "due_date": t.due_date.isoformat() if t.due_date else None,
                "tags": [tag.name for tag in t.tags] if t.tags else []
            }
            for t in tasks
        ]

        return {
            "success": True,
            "count": len(task_list),
            "tasks": task_list
        }
    except Exception as e:
        return {"success": False, "message": f"Failed to search tasks: {str(e)}"}
